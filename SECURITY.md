...
- Sensitive data exposure (credentials, PII, tenant data, privileged legal documents)
- Privilege escalation
- API security issues (broken object-level authorization, rate limiting bypass)
- Audit trail tampering or integrity hash bypass
- Legal hold data exposure or unauthorized release
...
4. Never log sensitive data (tokens, passwords, PII, privileged document content)
...
7. Audit trail entries are immutable — never implement any modification path for `lgl_audit_trails`
